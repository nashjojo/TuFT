...
Implements BaseTrainingBackend; uses MultiAdapterFSDPWorker with the native
engine in tuft.backends.fsdp_engine (no verl dependency).
"""
...
from tuft.backends.fsdp_engine import (
    FSDPEngineConfig,
    build_causal_lm,
    compute_num_micro_batches,
    forward_backward_batch,
    shard_list as _shard_list,
)
...
def _config_to_worker_dict(config: ModelConfig) -> dict:
    """ModelConfig -> serializable dict for FSDPEngineConfig and SlotPoolConfig in actors."""
    ...
    return {
        ...
        # not specify one. None lets transformers pick its own default.
...
def _worker_dict_to_configs(config_dict: dict) -> tuple[FSDPEngineConfig, SlotPoolConfig]:
    """Build FSDPEngineConfig and SlotPoolConfig from config_dict inside an actor."""
    ...
    attn_implementation = override.pop("attn_implementation", None) or (
        config_dict.get("attn_implementation") or "eager"
    )
    ...
    return engine_config, slot_config


# =============================================================================
# MultiAdapterFSDPWorker
# =============================================================================
# Holds the FSDP-2 sharded PEFT model; manages LoRA slot allocation/release; before
# forward_backward calls set_adapter and runs the native micro-batch loop; provides
# per-adapter optim_step/save/load.
# No Ray dependency. Backend holds one instance in single-process mode; in multi-GPU mode,
# N FSDPWorkerActor (Ray) instances each hold one; N processes form torch.distributed FSDP;
...
class MultiAdapterFSDPWorker:
    """
    Multi-LoRA slots: holds the sharded module and runs forward_backward_batch;
    ...
    def initialize(self) -> None:
        """Build model (base + PEFT adapters + FSDP v2) and create per-adapter optimizers."""
        if self._initialized:
            return
        base_model = build_causal_lm(self.engine_config)
    ...
    def forward_backward(
        ...
        """Set PEFT active adapter, then run the native micro-batch loop (no step).
        ...
        This is required for two cases:
          1. Per-call internal micro-batch accumulation (a call is split into
             num_micro_batches chunks and gradients persist across the loop).
    ...
    def release_adapter_grads(self, adapter_name: str) -> None:
        """Drop any accumulated gradients for the adapter. Call on adapter release."""
        if self.module is not None and adapter_name in self._adapters:
...
class FSDPWorkerActor:
    ...
    def build_worker(self) -> None:
        if self._worker is not None:
            return
        engine_config, slot_config = _worker_dict_to_configs(self.config_dict)
        self._worker = MultiAdapterFSDPWorker(
            engine_config=engine_config,
            slot_config=slot_config,
        )
        self.logger.info("[FSDPWorkerActor] build_worker: initializing rank=%s", self.rank)
        self._worker.initialize()
        self.logger.info("[FSDPWorkerActor] build_worker: initialized rank=%s", self.rank)
    ...
    def forward_backward(
        ...
        num_micro_batches: how many micro-batches to split the shard into.
        The backend computes ONE value from all shard sizes and passes it to
        every actor: each backward triggers FSDP-2 gradient collectives, so
        all data-parallel ranks must run the same number of micro-batches per
        RPC or NCCL deadlocks. Gradients accumulate across micro-batches; the
        caller is responsible for invoking `optim_step` afterwards (which will
        step + zero_grad).
        """
    ...
    def release_adapter_grads(self, adapter_name: str) -> None:
        """Drop accumulated gradients for the adapter on this actor; call on release."""
        if self._worker is not None:
            self._worker.release_adapter_grads(adapter_name)


# =============================================================================
# FSDPTrainingBackend (implements BaseTrainingBackend)
# Multi-GPU: N GPUs = N FSDPWorkerActor (Ray), forming torch.distributed.
# =============================================================================
...
class FSDPTrainingBackend(BaseTrainingBackend):
    ...
    Uses N FSDPWorkerActor (Ray), one per GPU, forming a process group for FSDP.
    """
    ...
    async def async_init(self) -> None:
        ...
        if not use_ray:
            ...
            engine_config, _slot = _worker_dict_to_configs(self._config_dict)
            self._worker = MultiAdapterFSDPWorker(
                engine_config=engine_config,
        ...
        async with self._lock:
            ...
            if adapter_name:
                ...
                if self._worker is not None:
                    ...
                    await asyncio.to_thread(
                        ray.get,
                        [a.release_adapter_grads.remote(adapter_name) for a in self._actors],
                    )
    ...
    async def forward(
        ...
        # micro-batches. Each micro-batch runs a full forward+backward, and
        # gradients accumulate across micro-batches (nothing zero_grads until
        # the user-invoked optim_step).
        #
        # The number of micro-batches is computed ONCE here from all shard
        # sizes and passed to every worker: each backward triggers FSDP-2
        # gradient collectives, so all data-parallel ranks must run the same
        # count per call or NCCL deadlocks (e.g. a batch of 7 over 2 GPUs
        # with micro_batch_size=1 would otherwise run 4 vs 3 micro-batches).
        #
        ...
        if self._worker is not None:
            n_micro = compute_num_micro_batches([len(data)], mb)
            ...
            loss_fn_outputs = out.get("loss_fn_outputs", [])
        else:
            import ray
            ...
            n_micro = compute_num_micro_batches([len(s) for s in shards], mb)
            self.logger.info(
                "FSDP multi-actor forward: batch=%d actors=%d micro_batches_per_actor=%d",
                len(data),
                n_actors,
                n_micro,
            )
